...
namespace gp_lio{
    ...
    PointCloudT Frame::GetTransformedCloud(ExternalParameters &extrinsic_parameters) {
        PointCloudT cloud;

        if (USE_SURF_FACTOR){
            PointCloudT surf_cloud = GetTransformedSurfCloud(extrinsic_parameters);
            cloud += surf_cloud;
        }
        if(USE_EDGE_FACTOR){
            PointCloudT edge_cloud = GetTransformedEdgeCloud(extrinsic_parameters);
            cloud += edge_cloud;
        }
