/*
 * @Descripttion: 
 * @version: 
 * @Author: luo
 * @Date: 2021-11-04 19:51:49
 * @LastEditors: luo
 * @LastEditTime: 2021-11-18 17:46:23
 */
#include "kalman_filter_pkg/subscriber/velocity_subscriber.hpp"

// #include "glog/logging.h"

namespace KalmanFilter{
VelocitySubscriber::VelocitySubscriber(ros::NodeHandle& nh, std::string topic_name, size_t buff_size)
    :nh_(nh) {
    subscriber_ = nh_.subscribe(topic_name, buff_size, &VelocitySubscriber::msg_callback, this);
}

void VelocitySubscriber::msg_callback(const geometry_msgs::TwistStampedConstPtr& twist_msg_ptr) {
    buff_mutex_.lock();
    VelocityData velocity_data;
    velocity_data.time = twist_msg_ptr->header.stamp.toSec();

    velocity_data.linear_vectority.x = twist_msg_ptr->twist.linear.x;
    velocity_data.linear_vectority.y = twist_msg_ptr->twist.linear.y;
    velocity_data.linear_vectority.z = twist_msg_ptr->twist.linear.z;

    velocity_data.angular_vectory.x = twist_msg_ptr->twist.angular.x;
    velocity_data.angular_vectory.y = twist_msg_ptr->twist.angular.y;
    velocity_data.angular_vectory.z = twist_msg_ptr->twist.angular.z;

    new_velocity_data_.push_back(velocity_data);
    buff_mutex_.unlock();
}

void VelocitySubscriber::ParseData(std::deque<VelocityData>& velocity_data_buff) {
    buff_mutex_.lock();
    if (new_velocity_data_.size() > 0) {
        velocity_data_buff.insert(velocity_data_buff.end(), new_velocity_data_.begin(), new_velocity_data_.end());
        new_velocity_data_.clear();
    }
    buff_mutex_.unlock();
}
}