 ...
 * @version: 
 * @Author: luo
 * @Date: 2021-12-31 10:53:07
 * @LastEditors: luo
 * @LastEditTime: 2022-01-17 17:28:19
 */
//
...
{
    IMUOdompreintegration::IMUOdompreintegration(ros::NodeHandle& nh):nh_(nh) 
    {
        //lidar to imu
        ...
        //lidar to imu
        encoder_extrinsic_maritx_ = Eigen::Matrix4d::Identity();
        Eigen::Quaterniond encoder_q(encoder_extrinsic_maritx_.block<3,3>(0,0));
        Eigen::Vector3d encoder_t = encoder_extrinsic_maritx_.block<3,1>(0,3);
        // encoder_t[0] = 0.75;
        // encoder_t[2] = 2.0;
        lidar2encoder_ = gtsam::Pose3(gtsam::Rot3(encoder_q), gtsam::Point3(encoder_t.x(), encoder_t.y(), encoder_t.z()));
        encoder2lidar_ = lidar2encoder_.inverse();

        lio_opt_initialized_ = false;

        subImu_ = nh_.subscribe<sensor_msgs::Imu>("/imu/data_xsens",100,&IMUOdompreintegration::ImuCallBack,this);
        subPriorOdometry_ = nh_.subscribe<nav_msgs::Odometry>("/matching_odom",100,&IMUOdompreintegration::PriroOdometeyCallBack,this);
        encoder_sub_ = nh_.subscribe<nav_msgs::Odometry>("/synced_encoder_odom",100, &IMUOdompreintegration::PriroEncoderOdomCallBack, this);


        pub_li_odom_ptr_ = std::make_shared<OdometryPublisher>(nh_,"/localization/gtsam_fusion","/map","lidar",100);
        ...
        encoder_correctionNoise_ = gtsam::noiseModel::Diagonal::Sigmas((gtsam::Vector(6) << 0.05, 0.05, 0.05, 0.1, 0.1, 0.1).finished());   // rad,rad,rad,m, m, m
        encoder_correctionNoise2_ = gtsam::noiseModel::Diagonal::Sigmas((gtsam::Vector(6) << 1, 1, 1, 1, 1, 1).finished()); 
        ...
        // std::thread*command(new std::thread(&IMUOdompreintegration::Command,this));
    }

    IMUOdompreintegration::~IMUOdompreintegration() {}

    void IMUOdompreintegration::run() {
    ...
    bool IMUOdompreintegration::HasData() {
        return !(prior_odom_buffer_.empty() || imu_opt_buffer_.empty() 
                                            || imu_odom_buffer_.empty() || prior_encoder_odom_buffer_.empty());
    }

    bool IMUOdompreintegration::ValidData() {
        mutex_.lock();
        current_prior_odom_pose_ = prior_odom_buffer_.front();
        cur_encoder_prior_odom_pose_ = prior_encoder_odom_buffer_.front();

        std::cout << " ************gtsam_node*****ValidData******* " << std::endl;
        std::cout << " prior_odom_buffer_ size = " << prior_odom_buffer_.size()<< std::endl;
        std::cout << " prior_encoder_odom_buffer_ size = " << prior_encoder_odom_buffer_.size()<< std::endl;

        double diff_time = cur_encoder_prior_odom_pose_->header.stamp.toSec() - current_prior_odom_pose_->header.stamp.toSec();
        ...
        if(diff_time > 0.05)
        {
            prior_encoder_odom_buffer_.pop_front();

            return false;
        }

        prior_encoder_odom_buffer_.pop_back();        
        prior_odom_buffer_.pop_front();

        // std::cout << " prior_odom_buffer_ 22222222222 size = " << prior_odom_buffer_.size()<< std::endl;
        // std::cout << " prior_encoder_odom_buffer_ 222222222 size = " << prior_encoder_odom_buffer_.size()<< std::endl;

        mutex_.unlock();
        return true;
    }

    bool IMUOdompreintegration::UpdatePreintegration() {

        std::cout << " ************gtsam_node*****UpdatePreintegration******* " << std::endl;
        // std::cout << " prior_odom_buffer_ size = " << prior_odom_buffer_.size()<< std::endl;
        // std::cout << " prior_encoder_odom_buffer_ size = " << prior_encoder_odom_buffer_.size()<< std::endl;
        ...
        float encode_p_x = cur_encoder_prior_odom_pose_->pose.pose.position.x;
        float encode_p_y = cur_encoder_prior_odom_pose_->pose.pose.position.y;
        float encode_p_z = cur_encoder_prior_odom_pose_->pose.pose.position.z;
        float encode_r_x = cur_encoder_prior_odom_pose_->pose.pose.orientation.x;
        float encode_r_y = cur_encoder_prior_odom_pose_->pose.pose.orientation.y;
        float encode_r_z = cur_encoder_prior_odom_pose_->pose.pose.orientation.z;
        float encode_r_w = cur_encoder_prior_odom_pose_->pose.pose.orientation.w;
        bool encoder_degenrate = (int)cur_encoder_prior_odom_pose_->pose.covariance[0] == 1 ? true : false;
        gtsam::Pose3 encoder_prior_pose = gtsam::Pose3(
                                    gtsam::Rot3::Quaternion(encode_r_w, encode_r_x, encode_r_y, encode_r_z),
                                    gtsam::Point3(encode_p_x, encode_p_y, encode_p_z));
        std::cout << " *****************UpdatePreintegration****initialize*** " << std::endl;
        // 0. initialize system
        if(lio_opt_initialized_ == false)
        {
            last_encoder_prior_odom_pose_ = encoder_prior_pose;
            ...
            //initial pose
            nav_msgs::Odometry cur_odom = *prior_odom_buffer_.back();
            Eigen::Quaterniond qua;
            qua.w() = cur_odom.pose.pose.orientation.w;
            qua.x() = cur_odom.pose.pose.orientation.x;
            qua.y() = cur_odom.pose.pose.orientation.y;
            qua.z() = cur_odom.pose.pose.orientation.z;
            Eigen::Vector3d trans_p(cur_odom.pose.pose.position.x, cur_odom.pose.pose.position.y, cur_odom.pose.pose.position.z);
            gtsam::Pose3 pose_p = gtsam::Pose3(gtsam::Rot3(qua),gtsam::Point3(trans_p.x(),trans_p.y(),trans_p.z()));
            prevPose_ = lidarPriorPose.compose( lidar2Imu_) * pose_p;
...
//            imuIntegratorImu_->resetIntegrationAndSetBias(prevBias_);
        ...
        std::cout << " *****************UpdatePreintegration****key == 100*** " << std::endl;
        ...
        std::cout << " *****************UpdatePreintegration****add_imu_factor*** " << std::endl;
        ...
        // std::cout << " *****************UpdatePreintegration****add_odom_factor*** " << std::endl;   
        // // add odom pose factor
        // gtsam::noiseModel::Diagonal::shared_ptr encoder_odom_noise = gtsam::noiseModel::Diagonal::Variances(
        //         (gtsam::Vector(6) << 1e-4, 1e-4, 1e-4, 1e-4, 1e-4, 1e-4).finished()
        // );
        // gtsam::Point3 delta_p = last_encoder_prior_odom_pose_.translation() - encoder_prior_pose.translation();
        // gtsam::Rot3 delta_q = last_encoder_prior_odom_pose_.rotation() * encoder_prior_pose.rotation();

        // gtsam::Pose3 delta_pose = gtsam::Pose3(delta_q, delta_p);
        // gtsam::Pose3 cur_odom_pose = delta_pose.compose(encoder2lidar_);
        // graphFactors_li_.add(gtsam::BetweenFactor<gtsam::Pose3>(X(key -1), X(key), cur_odom_pose, encoder_odom_noise));

        // last_encoder_prior_odom_pose_ = encoder_prior_pose;


        std::cout << " *****************UpdatePreintegration****add_pose_factor*** " << std::endl;                  
        // add pose factor
        // 添加当前关键帧位姿因子
        ...
        std::cout << " *****************UpdatePreintegration****insert predicted values*** " << std::endl;  
    ...
    bool IMUOdompreintegration::failureDetection(const gtsam::Vector3 &velCur,
                                             const gtsam::imuBias::ConstantBias &biasCur)
    {
        ...
        // {
        //     std::cout << "ba.norm = " << ba.norm() << std::endl;
        //     std::cout << "bg.norm = " << bg.norm() << std::endl;
    ...
    void IMUOdompreintegration::resetOptimization()
    {
        // 重置isam2优化器
    ...
    void IMUOdompreintegration::resetParams()
    {
        lastImuTime_opt_ = -1;
        doneFirstOpt_ = false;
        lio_opt_initialized_ = false;
    ...
    bool IMUOdompreintegration::PublishData() {
    ...
    void IMUOdompreintegration::ImuCallBack(const sensor_msgs::ImuConstPtr &imu_raw) {
        mutex_.lock();
...
//        // 发布通过imu估计的雷达里程计信息(后面都称为imu里程计信息)
    ...
    void IMUOdompreintegration::PriroOdometeyCallBack(const nav_msgs::OdometryConstPtr &odomMsg) {
        mutex_.lock();
        prior_odom_buffer_.push_back(odomMsg);
        mutex_.unlock();
    }

    void IMUOdompreintegration::PriroEncoderOdomCallBack(const nav_msgs::OdometryConstPtr &encoder_msg)
    {
        mutex_.lock();

        prior_encoder_odom_buffer_.push_back(encoder_msg);

        mutex_.unlock(); 
    }

//     void IMUOdompreintegration::Command() {
//         while (1){
//             char c = std::getchar();
//             if('s'==c){
//                 std::cout <<"get command s ..."<<std::endl;
// //                std::string dir_path = MAP_PATH + "/odom1";
//                 std::string dir_path = "/home/luo/work/filter_ws/src/kalman_filter_pkg/result_map/result/odom1";
