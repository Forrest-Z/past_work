 ...
 * @version: 
 * @Author: luo
 * @Date: 2021-12-23 15:19:31
 * @LastEditors: luo
 * @LastEditTime: 2022-01-17 17:45:31
 */
#pragma once 
...
public:
    ...
    void SetResIMUPreIntrgration(const ceres::CostFunction *residual, const std::vector<double *> &parameter_blocks)
    {
        //init
        ResidualBlockInfo res_imu_pre_intrgration(residual, parameter_blocks);
        Eigen::VectorXd residuals;
        std::vector<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> jacobians;

        //compute
        Evaluate(res_imu_pre_intrgration, residuals, jacobians);
        const Eigen::MatrixXd &J_m = jacobians.at(0);
        const Eigen::MatrixXd &J_r = jacobians.at(1);
    ...
    void Marginalize(const double *raw_param_r_0)
    {
        ...
        x_0_ = x_0;
            // std::cout << "------ Marginalize----00----- " << std::endl;
        //marginalize
        ...
        J_ = S_sqrt.asDiagonal() * saes.eigenvectors().transpose();
                    // std::cout << "------ Marginalize----66----- =  " << J_ << std::endl;
...
private:
    ...
    struct ResidualBlockInfo
    {
        ...
        ResidualBlockInfo(const ceres::CostFunction *_residual, const std::vector<double *> &_parameter_blocks)
    ...
    static void Evaluate(ResidualBlockInfo &residual_info, Eigen::VectorXd &residuals, 
                        std::vector<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> &jacobians)
    {
