...
float ReqSteer;
float ReqSpeed = 0.0;
float ReqSteer_compute;
float ReqSteer_tune;
//vector<double> ReqSteer_c ;

//注意这里的变量定义的位置！！

ros::Publisher can_pub; //这里预先定义好一个can_pub，以实现在回调函数中，对CAN线进行发送；这里是直接发送到can_rx的topic上；
...
void steer_Callback(const geometry_msgs::Twist::ConstPtr &msg) {

   ReqSteer_compute = msg-> angular.z *19.53;

//   ReqSteer_c.push_back(ReqSteer_compute);    
  }                                         
...
void dbc_tx_Callback(const can_msgs::Frame::ConstPtr &msg) {
  ...
  if (msg->id == 0x504) {
    ...
    ROS_INFO("actual steeringangle is: %f ",IMCU_msg.IMCU504_Fbsteeringangle);    
...
//    ReqSteer_compute = ReqSteer_c.back(); 
//    ReqSteer_compute = 300;
//    ReqSteer_tune = -ReqSteer_compute;
    if (abs(IMCU_msg.IMCU504_Fbsteeringangle - (-ReqSteer_compute)) < 300)
    {
        ReqSteer_tune = -ReqSteer_compute;
       ROS_INFO("ReqSteer_tune 0 is: %f",ReqSteer_tune);
    }
    else
    {
        if( ReqSteer_compute > 0)
        {
            ReqSteer_tune = IMCU_msg.IMCU504_Fbsteeringangle + (-299.9);
            ROS_INFO("ReqSteer_tune 1 is: %f",ReqSteer_tune);
        }
        else if(ReqSteer_compute < 0)
        {
            ReqSteer_tune = IMCU_msg.IMCU504_Fbsteeringangle + 299.9;
            ROS_INFO("ReqSteer_tune -1 is : %f",ReqSteer_tune);
        }
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber steer_sub = n.subscribe("/cmd_delta", 100, steer_Callback);
  ...
  while (ros::ok()) {
    ...
    can_501_1.UC501_Reqsteeringangle = ReqSteer_tune;
