...
namespace gp_lio{
    ...
    current_scan_(new pcl::PointCloud<pcl::PointXYZI>){
        ...
        has_inited_ = false;
        // cam2lidar_ << 0 , 0, 1, 0.055,
        //               -1, 0, 0, 0.035,
        //               0 ,-1, 0,-0.06,
        //               0 , 0, 0, 1 ;
        cam2lidar_ << 0 , 0, 1, 0.4634345,
                -1, 0, 0, 0.0000001,
                0 ,-1, 0,-0.300378104,
                0 , 0, 0, 1 ;
    ...
    bool Localization::SetInitPose(const Eigen::Matrix4f& init_pose) {
        ...
        std::cout << "SetInitPose--->" << tmp_pose<<std::endl;
...
std::cout<< "time 1 = " << (double)(t1 - t0) /  CLOCKS_PER_SEC << std::endl;
...
t2= clock();
std::cout<< "time 2 = " << (double)(t2 - t1) /  CLOCKS_PER_SEC << std::endl;
    ...
    bool Localization::GaussPoseByAruco(ArucoData local_aruco_data,Eigen::Matrix4f& current_gauss_pose){
        ArucoData global_aruco_data;
        Eigen::Matrix4f global_aruco_pose,local_aruco_pose;
        local_aruco_pose = local_aruco_data.getPose();
        std::cout << "GaussPoseByAruco ---> local_aruco_pose_:\n" <<local_aruco_data.id<<" "<< local_aruco_pose<<std::endl;
        if(!map_manager_.GetGlobalArucoPose(local_aruco_data.id,global_aruco_data))
        {
            std::cout << "not this aruco id data in global map" << std::endl;
            return false;
        }
        global_aruco_pose = global_aruco_data.getPose();
        std::cout << "GaussPoseByAruco ---> global_aruco_pose_:\n" <<global_aruco_data.id<<" "<< global_aruco_pose<<std::endl;
        current_gauss_pose_ = global_aruco_pose * (local_aruco_pose.inverse()) * (cam2lidar_.inverse());
        current_gauss_pose = current_gauss_pose_;
        std::cout << "GaussPoseByAruco ---> current_gauss_pose_:\n" << current_gauss_pose_<<std::endl;
        SetInitPose(current_gauss_pose_);
        has_inited_ = true;
        std::cout << "Localization Aruco Inited Finished" <<std::endl;
        return true;
