...
NodeId PoseGraph2D::AddNode(
  ...
  t1 = clock();
  std::cout << "----------AddNode->GetLocalToGlobalTransform time = " <<(double)(t1 -t0) / CLOCKS_PER_SEC << std::endl;

  const NodeId node_id = AppendNode(constant_data, trajectory_id,
                                    insertion_submaps, optimized_pose);
  
  t2 = clock();
  std::cout << "----------AddNode->AppendNode time = " <<(double)(t2 -t1) / CLOCKS_PER_SEC << std::endl;
  ...
  t3 = clock();
std::cout << "----------AddNode->ComputeConstraintsForNode time = " <<(double)(t3 -t2) / CLOCKS_PER_SEC << std::endl;
  return node_id;
}
