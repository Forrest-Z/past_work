...
void trajectory_callback(const tracking_pid::traj_point& goalPointMsg)
{
  ...
  try
  {
    ...
    threshold = goalPointMsg.velocity.linear.x;
  }
...
void poseCallback()
{
  ...
  if (!waiting_for_setpoint)
  {
    ...
    else if( vel_x < -vel_threshold) 
    {
      vel_x = -vel_threshold;
    }else if(std::fabs(vel_x) < 0.05)
    {
      vel_x = 0.05 * (vel_x>0 ? 1.0:-1.0);
    }

    if(w_z  > w_threshold)
    {
      w_z = w_threshold;
    }else if( w_z < - w_threshold)
    {
      w_z = -w_threshold;
    }else if(std::fabs(w_z) < 0.157 )
    {
      w_z = 0.157 * (w_z>0 ? 1.0:-1.0);
    }
